...
async def delete_booking_mock_response(request: Request, mock: str = Query(...)) -> Response:
    ...
    with vcr.use_cassette(f"cassettes/delete_booking/{cassette_file}.yaml") as cassette:
        request_data = {
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Request data does not match existing cassette for booking deletion."
            ) from exc
